...
const ReservationVehicleCard = ({ vehicleBooking }: { vehicleBooking: ResponseForProfilePage }) => {
    ...
    const { model, brand, year, capacity, fuelType, transmissionType, images } = vehicle || {};
    const { imageUrl } = qrCode?.[0] || {};

    const formattedStart = String(startDate).split("T")[0];
    const formattedEnd = String(endDate).split("T")[0];
    const days =
        formattedStart && formattedEnd ? Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) : "-";
    const formattedTotal = `${totalPrice.toFixed(2)}€`;

    const subTotal = pricePerDay * calculateTotalDays(String(startDate), String(endDate));
                                    ...
                                    <div className="flex flex-col gap-2 items-center">
                                        ...
                                        <AnimatePresence>
                                            {showCancelWarning && (
                                                ...
                                                >
                                                    <p className="mb-2">
                                                        Al cancelar tu reserva podrías perder tu lugar. La disponibilidad es limitada y no
                                                        garantizamos que queden plazas más adelante.
                                                    </p>
