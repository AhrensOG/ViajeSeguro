...
type Props = {
  ...
  setReservation: React.Dispatch<
    React.SetStateAction<ReservationResponse | null>
  >;
};
    ...
    try {
      ...
      if (!reservation.qr?.id) {
        toast.error("No se pudo actualizar el estado del QR.", {
          description:
            "El ID del QR es requerido. Intenta nuevamente o contacta con el soporte.",
        });
        return;
      }
  ...
  return (
    <section className="text-sm bg-custom-gray-100 p-4 rounded-lg flex flex-col gap-3">
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="font-medium text-custom-black-900">
          {isCash ? "Efectivo" : "Stripe"}
        </span>
      </div>
