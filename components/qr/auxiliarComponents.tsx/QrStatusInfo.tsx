...
const QrStatusInfo = ({ reservation }: Props) => {
  const qr = reservation.qr;

  return (
    <section className="bg-custom-gray-100 p-4 rounded-lg text-sm">
      <p className="font-medium text-custom-black-900 mb-1">
        ...
        <div className="flex flex-col gap-2 text-red-600 font-medium mt-1">
          ...
          {qr?.usedAt ? (
            <span className="text-red-500 font-medium">
              Ya fue escaneado el {formatDateTime(qr.usedAt)}
            </span>
          ) : (
            <span className="text-red-500 font-medium">
              El código QR no esta activo.
            </span>
          )}
        </div>
      )}
