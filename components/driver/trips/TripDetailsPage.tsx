...
const TripDetailsPage = () => {
            ...
            try {
                ...
                const mapped: TripDetailsResponse = {
                    ...
                    passengers: (raw.reservations || []).map((res: Passengers) => {
                        const validStatuses: ValidStatus[] = ["PENDING", "CONFIRMED", "CANCELLED"];
                        const status: ValidStatus = validStatuses.includes(res.status as ValidStatus) ? (res.status as ValidStatus) : "PENDING";
                        return {
                            id: res.id,
                            userId: res.user?.id ?? "",
                            fullName: `${res.user?.name ?? ""} ${res.user?.lastName ?? ""}`.trim(),
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 md:p-6">
            <h1 className="text-2xl font-bold mb-6 text-custom-black-900">Detalles del viaje</h1>
            ...
            <div className="bg-custom-white-100 shadow-md rounded-xl p-6 border border-custom-gray-200">
                ...
                <p className="text-sm text-custom-gray-700">
                    <strong>Capacidad:</strong> {trip.capacity} — <strong>Mínimo:</strong> {trip.minPassengers}
                </p>
            ...
            <div className="mt-8">
                <h3 className="text-lg font-semibold text-custom-black-900 mb-3">Pasajeros</h3>
                {trip.passengers.length === 0 ? (
                    <p className="text-sm text-custom-gray-600 italic">No hay pasajeros en este viaje aún.</p>
                ) : (
                        ...
                        <table className="min-w-full text-sm text-left">
                            ...
                            <tbody>
                                {trip.passengers.map((passenger) => (
                                    <tr key={passenger.id} className="border-t border-custom-gray-100 hover:bg-custom-golden-50">
