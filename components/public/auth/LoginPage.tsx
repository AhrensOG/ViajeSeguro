...
import LogInForm from "./auxiliarComponents/login/LogInForm";
import React, { Suspense } from "react";
import LoginFormSkeleton from "./auxiliarComponents/login/LoginFormSkeleton";
import { useSearchParams } from "next/navigation";

const LoginPage = () => {
  const searchParams = useSearchParams();
  const query = searchParams.toString();
  const registerLink = `/auth/register${query ? `?${query}` : ""}`;
