...
export default function FiltersBar() {
  ...
  const handleModeClick = (selected: "car" | "van") => {
    ...
    if (selected === "car") {
      // Copiamos lo útil y NORMALIZAMOS serviceType para 'car'
      if (searchParams.get("origin"))
        sp.set("origin", searchParams.get("origin")!);
      if (searchParams.get("destination"))
        sp.set("destination", searchParams.get("destination")!);
      if (searchParams.get("departure"))
        sp.set("departure", searchParams.get("departure")!);
      sp.set("serviceType", "SIMPLE_TRIP"); // <-- clave
    } else {
      // Para 'van' usamos SIEMPRE el estado controlado (válido) de van
      ...
      sp.set("serviceType", serviceType); // <-- clave (WITH_DRIVER / WITHOUT_DRIVER)
    }
  ...
  const handleSearch = (data: ClientSearchFormData) => {
    if (!data.origin || !data.destination || !data.departure) {
      toast.warning(
        "Por favor, completa todos los campos obligatorios para buscar."
      );
      return;
    }
  ...
  const handleSearchVan = () => {
    ...
    if (returnDate < pickupDate) {
      toast.info(
        "La fecha de devolución no puede ser anterior a la fecha de recogida."
      );
      return;
    }
        ...
        <div className="w-full">
          <SearchForm
            initialData={{
              ...
              serviceType:
                (searchParams.get("serviceType") as "SIMPLE_TRIP") ||
                "SIMPLE_TRIP",
