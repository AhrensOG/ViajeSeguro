...
import { sendContactFormFromContactPage } from "@/lib/api/contact";
...
const ContactForm = () => {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      await sendContactFormFromContactPage(data);
      toast.success("Mensaje enviado con éxito", { id: toastId });
      setIsSubmitted(true);
      reset();
    } catch {
      toast.error("Ocurrió un error al enviar el mensaje", { id: toastId });
    }
  };
      ...
      <AnimatePresence mode="wait">
        {!isSubmitted ? (
          <motion.form
            ...
            className="space-y-6">
            <div>
              <label className="block text-custom-gray-800 mb-1 font-medium">
                Nombre completo
              </label>
              <input
                type="text"
