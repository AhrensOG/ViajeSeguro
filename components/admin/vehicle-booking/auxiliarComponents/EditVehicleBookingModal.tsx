...
const EditVehicleBookingModal = ({ onClose, onSuccess, booking, renters, offers }: Props) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const submit: SubmitHandler<FormData> = async (data) => {
        try {
            ...
            const res = await fetch("/api/admin/vehicle-bookings", {
                method: "PUT",
            ...
            >
                ...
                <form onSubmit={handleSubmit(submit)} className="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm text-custom-black-800">
                        ...
                        <select {...register("offerId", { required: true })} className={inputClass}>
                            ...
                            {offers.map((o) => (
                                <option key={o.id} value={o.id}>
                                    {o.withdrawLocation} → {o.returnLocation} (€{o.pricePerDay}/día)
                                </option>
                            ))}
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" {...register("totalPrice", { required: true, min: 0 })} className={inputClass} />
                        {errors.totalPrice && <p className="text-red-500 text-xs">Campo obligatorio</p>}
                    </div>
