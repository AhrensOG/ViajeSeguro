...
interface DeliveryCaptureModalAdminProps {
    isOpen: boolean;
...
export default function DeliveryCaptureModalAdmin({ isOpen, bookingId, onClose, onComplete, phase = 'OWNER_PRE' }: DeliveryCaptureModalAdminProps) {
    ...
    const handleUploadAndContinue = async () => {
        ...
        try {
            ...
            toast.success(phase === 'OWNER_PRE' ? "Fotos de entrega subidas correctamente" : "Fotos de recepción subidas correctamente");
            onComplete(urls);
            onClose();
        } catch (e) {
            console.error(e);
    ...
    const title = phase === 'OWNER_POST' ? "Registro fotográfico de devolución (Admin)" : "Registro fotográfico de entrega (Admin)";

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
            <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-custom-golden-50">
                    <h3 className="text-xl font-semibold text-custom-golden-700">{title}</h3>
                    <button onClick={onClose} className="p-2 rounded-lg hover:bg-custom-golden-100" aria-label="Cerrar">
                        <X className="h-5 w-5 text-custom-golden-700" />
                    </button>
                </div>
                ...
                <div className="p-4 grid gap-4">
                    {stepIndex === -1 && (
                        <div className="space-y-4">
                            <div className="rounded-xl border border-custom-golden-200 bg-custom-golden-50 p-5">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-custom-golden-500/30"
                            ...
                            <div className="flex justify-end">
                                <button
                                    ...
                                    className="px-4 py-2 bg-custom-golden-600 text-white rounded-lg shadow-sm hover:bg-custom-golden-700"
                                >
                                    Continuar
                                </button>
                            </div>
                    ...
                    {stepIndex >= 0 && (
                        <>
                            <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                <div
                                    className="h-full bg-custom-golden-600 transition-all"
                                    style={{ width: `${((stepIndex) / (CAPTURE_STEPS.length)) * 100}%` }}
                                />
                            </div>

                            <div className="rounded-xl border border-custom-golden-200 bg-custom-golden-50 p-4">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm font-semibold text-gray-900">Paso {stepIndex + 1} de {CAPTURE_STEPS.length}</p>
                                    <span className="text-xs text-custom-golden-800 px-2 py-1 rounded-full bg-white border border-custom-golden-200">{CAPTURE_STEPS[stepIndex].label}</span>
                                </div>
                                <p className="mt-2 text-sm text-gray-800">
                                    {CAPTURE_STEPS[stepIndex].hint}
                                    {currentStepIsOptional && <span className="block font-bold mt-1 text-custom-golden-800">(Este paso es opcional)</span>}
                                </p>
                            </div>
                                        ...
                                        <>
                                            ...
                                            <div className="absolute bottom-3 left-0 right-0 flex items-center justify-center gap-3">
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 px-5 py-2.5 bg-custom-golden-600 text-white rounded-full shadow-sm hover:bg-custom-golden-700 disabled:opacity-50"
                                                >
                                        ...
                                        <>
                                            ...
                                            <div className="absolute bottom-3 left-0 right-0 flex items-center justify-center gap-3">
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-white text-gray-800 rounded-full hover:bg-custom-golden-50 border border-custom-golden-200"
                                                >
                                                    Repetir foto
                                                </button>
                                                {stepIndex < CAPTURE_STEPS.length - 1 && (
                                                    <button
                                                        ...
                                                        className="px-4 py-2 bg-custom-golden-600 text-white rounded-full shadow-sm hover:bg-custom-golden-700"
                                                    >
                                                        Siguiente foto
                                                    </button>
                                                )}
                ...
                {stepIndex >= 0 && capturedByStep.filter(Boolean).length >= MIN_PHOTOS && (
                    <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-100">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-custom-golden-600 text-white rounded-lg hover:bg-custom-golden-700 disabled:opacity-50 shadow-sm"
                        >
                            {isUploading ? (
                                <>
