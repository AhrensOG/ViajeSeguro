...
import {
  ...
  Partner,
  TripResponse,
} from "@/lib/api/admin/trips/trips.type";
  ...
  useEffect(() => {
    ...
    const fetchDrivers = async () => {
      try {
        ...
        if (!Array.isArray(res))
          throw new Error(
            "La respuesta del backend no es un array de conductores"
          );
        setDrivers(res);
      } catch {
    ...
    const fetchPartners = async () => {
      try {
        ...
        if (!Array.isArray(res))
          throw new Error(
            "La respuesta del backend no es un array de propietarios"
          );
        setPartners(res);
      } catch {
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden pb-2">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
        <h1 className="text-2xl font-bold text-custom-golden-600">
          Panel de Viajes
        </h1>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-3 bg-custom-white-100 p-4 border border-custom-gray-300 rounded-md mb-4">
        ...
        <input
          ...
          className="w-full border border-custom-gray-300 rounded-md px-4 py-2"
          onChange={(e) =>
            setFilters((f) => ({ ...f, arrival: e.target.value }))
          }
        />
        <input
          type="number"
