...
export default function TripsPanel() {
    ...
    const filteredTrips = trips.filter((trip) => {
        ...
        const matchesOrigin = filters.origin.toLowerCase() ? trip.origin.includes(filters.origin.toLowerCase()) : true;
        const matchesDestination = filters.destination.toLowerCase() ? trip.destination.includes(filters.destination.toLowerCase()) : true;
    ...
    return (
        <div className="w-full h-full flex flex-col overflow-hidden pb-2">
                        ...
                        <tbody className="text-custom-black-800">
                            {filteredTrips.map((trip, index) => (
                                ...
                                >
                                    ...
                                    <td onClick={(e) => e.stopPropagation()} className="px-4 py-2 border-b border-custom-gray-200 text-center space-x-2">
                                        <button
                                            onClick={() => {
                                                setSelectedTrip(trip);
            ...
            {isViewModalOpen && selectedTrip && <TripDetailsModal trip={selectedTrip} onClose={() => setIsViewModalOpen(false)} />}
            {isCreateModalOpen && (
                <CreateTripModal
