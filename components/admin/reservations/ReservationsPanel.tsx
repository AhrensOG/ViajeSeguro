...
import {
  ...
  TripOption,
  UserOption,
} from "@/lib/api/admin/reservation/reservation.types";
import ReservationDetailModal from "./auxiliarComponents/ReservationDetailsModal";
import {
  deleteRes,
  getAllReservations,
  getReduceTrip,
} from "@/lib/api/admin/reservation/intex";
...
export default function ReservationPanel() {
    ...
    const fetchReservations = async () => {
      try {
        ...
        if (!Array.isArray(trips))
          throw new Error("La respuesta de viajes no es un array");
        setTrips(
          trips.map((trip) => ({
            id: trip.id,
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden pb-2">
      <h1 className="text-2xl font-bold mb-4 text-custom-golden-600">
        Panel de Reservas
      </h1>
        ...
        <div className="flex-1 w-full bg-custom-white-100 rounded-xl shadow-sm border border-custom-gray-200 overflow-auto flex flex-col">
          <table className="min-w-full text-sm text-left table-fixed border-separate border-spacing-0">
            <thead className="bg-custom-golden-100 text-custom-golden-700 uppercase text-xs">
              <tr>
                <th className="px-4 py-2 border-b border-r border-custom-gray-300">
                  Nombre
                </th>
                <th className="px-4 py-2 border-b border-r border-custom-gray-300">
                  Email
                ...
                <th className="px-4 py-2 border-b border-custom-gray-300 text-center">
                  Acciones
                </th>
              </tr>
            </thead>
            <tbody className="text-custom-black-800">
              {filteredReservations.map((r, index) => {
                ...
                const capitalized =
                  formattedDeparture.charAt(0).toUpperCase() +
                  formattedDeparture.slice(1);
                ...
                return (
                  <tr
                    ...
                    <td className="px-4 py-2 border-b border-r border-custom-gray-200">
                      {capitalized || "Fecha no disponible"}
                    </td>
                    <td className="px-4 py-2 border-b border-r border-custom-gray-200">
                      € {r.price.toFixed(2)}
                    </td>
      ...
      {isEditingModalOpen && selectedReservation && (
        <EditReservationModal
          ...
          initialData={{
            ...
            paymentMethod: selectedReservation.paymentMethod as
              | "STRIPE"
              | "CASH"
              | "OTHER",
