...
export const createReservation = async (payload: CreateReservationPayload): Promise<ReservationConfirmationResponse> => {
...
export const getReservationsByUser = async (userId: string): Promise<ReservationResponse[]> => {
    return fetcher<ReservationResponse[]>(`${BACKEND_URL}/reservation/by-user/${userId}`);
};
...
export const markReservationAsPaid = async (reservationId: string): Promise<void> => {
    return fetchWithAuth<void>(`${BACKEND_URL}/reservation/mark-as-paid/${reservationId}`, {
        method: "PUT",
    });
};
